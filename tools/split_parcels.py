...
# split_parcels.py
#
#   [注記] このv1は tools/split_parcels_v2.py に統合され、現在は未使用です。
#   v1はセル割り当てを筆ポリゴンの「最初の頂点」のみで行っていたため、
#   1筆が複数メッシュにまたがる場合に境界の反対側から見ると地番が
#   欠落して見えるバグがあった（v2はbboxが重なる全セルへ出力する方式に
#   変更し、あわせて筆数に応じた可変グリッド分割にも対応した）。
#   本ファイルは経緯を追えるよう履歴として残している。
#
